...
import type { ResponsesRequest, ProviderName } from './types';
import { convertResponsesToChat, streamChatToResponses, convertChatToResponses } from './converter';
import { 
  getCurrentProvider, 
  getProvider, 
  getApiKey, 
...
${Object.entries(PROVIDERS).map(([k, p]) => 
  `║    ${k.padEnd(8)} - ${p.name.padEnd(15)} (tools: ${p.supportsTools ? '✓' : '✗'})`.padEnd(55) + '║'
).join('\n')}
╠════════════════════════════════════════════════════════╣
║  Codex Config:                                         ║
║  ~/.codex/config.toml:                                 ║
